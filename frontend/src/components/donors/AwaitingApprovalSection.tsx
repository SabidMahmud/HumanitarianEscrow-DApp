...
export default function AwaitingApprovalSection({
  ...
  const pending = missions.filter((m) => m.status === MissionStatus.AwaitingApproval);
    ...
    <section className="mb-12 animate-fade-in-up">
      ...
      <div className="grid gap-4">
        {pending.map((mission) => {
          ...
          return (
            <div key={mission.id} className="glass-panel rounded-2xl p-6 border-l-4 border-emerald-500">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1">
                    ...
                    <span className="font-mono text-slate-300">{truncate(mission.selectedAgency)}</span>
                  </p>
                </div>
                ...
                <div className="flex flex-col items-end gap-3 min-w-50">
                  ...
                  <div className="flex w-full gap-3">
                    ...
                    >
                      {isBusy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Gavel className="w-3.5 h-3.5" />}
                      Dispute
                    </button>
                    <button
