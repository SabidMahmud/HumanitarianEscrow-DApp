...
export default function DonorDashboardPage() {
  ...
  const [processingId, setProcessingId] = useState<number | "post" | null>(
    null,
  );
  ...
  const handlePostMission = (
    ...
    execTx(
      "post",
      (c) => c.postMission(category, parseEther(budget), region),
      "Mission posted!",
    ).then(() => setIsModalOpen(false));

  const handleFundMission = (
    missionId: number,
    agency: string,
    amount: bigint,
  ) =>
    execTx(
      missionId,
      (c) => c.fundMission(missionId, agency, { value: amount }),
      `Mission #${missionId} funded!`,
    );

  const handleApprove = (missionId: number) =>
    execTx(
      missionId,
      (c) => c.approveDelivery(missionId),
      `Delivery approved for mission #${missionId}.`,
    );

  const handleDispute = (missionId: number) =>
    execTx(
      missionId,
      (c) => c.disputeMission(missionId),
      `Dispute raised for mission #${missionId}.`,
    );
  ...
  const stats = {
    ...
    inTransit: missions.filter((m) => m.status === MissionStatus.InTransit)
      .length,
    awaitingApproval: missions.filter(
      (m) => m.status === MissionStatus.AwaitingApproval,
    ).length,
  };
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in-up">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-linear-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 px-5 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_rgba(245,158,11,0.3)] transition-all cursor-pointer"
            >
